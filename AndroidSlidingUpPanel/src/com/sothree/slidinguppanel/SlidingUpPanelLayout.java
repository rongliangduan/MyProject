...
public class SlidingUpPanelLayout extends ViewGroup {
		...
		if (attrs != null) {
			...
			if (ta != null) {
				mPanelHeight = ta.getDimensionPixelSize(
						R.styleable.SlidingUpPanelLayout_showedSize, -1);
				mShadowHeight = ta.getDimensionPixelSize(
						R.styleable.SlidingUpPanelLayout_shadowSize, -1);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (i == 1) {
				...
				// TODO 这里使 内容view 高度等于 父view，
				height = layoutHeight;
				// height -= panelHeight;
			}

			// Log.d(TAG, "======onMeasure====== child " + i + " , height="
			...
			// Log.d(TAG, "======onMeasure====== child " + i + " ,measure="
			// + childWidthSpec + " , " + childHeightSpec + " , height="
			// + height);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (lp.slideable) {
				...
				// TODO
				// 这里再减去ActionBar的高度，避免在ActionBar的windowActionBarOverlay为true时，slidePanel滑动超过ActionBar
				// int heightAb = getResources().getDimensionPixelSize(
				// R.dimen.abs__action_bar_default_height);
				// mSlideRange = mSlideRange - heightAb;

				// Log.d(TAG, "======onLayout====== mSlideRange=" + mSlideRange
				// + " ,childHeight=" + childHeight + " ,mPanelHeight="
				// + mPanelHeight);
			...
			// Log.d(TAG, "======onLayout====== child " + i + " ,layout="
			// + childLeft + " , " + childTop + " , " + childRight + " , "
			// + childBottom + " ,parentHeight=" + getHeight());
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		// if (ev.getY() < getResources().getDimensionPixelSize(
		// R.dimen.abs__action_bar_default_height)) {
		// return super.onTouchEvent(ev);
		// }
