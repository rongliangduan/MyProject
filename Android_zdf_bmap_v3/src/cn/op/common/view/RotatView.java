...
public class RotatView extends View {
    ...
    private void initSize() {
        ...
        o_x = (float)(maxwidth / 2);//确定圆心坐标
       
        int j=(int) (maxheight-height)/2;
       // System.out.println(maxheight+","+sh);
        o_y =(float)((maxheight) / 2);//确定圆心坐标
    ...
    protected void onDraw(Canvas canvas) {
       ...
       // matrix.setTranslate((float)width / 2, (float)height / 2);
        matrix.setTranslate((float)width / 2, (float)(height-sh)/ 2);
        // 开始转
        matrix.preRotate(deta_degree);
        // 转轴还原
        matrix.preTranslate(-(float)width / 2, -(float)(height-sh) / 2);

        // 将位置送到view的中心
        matrix.postTranslate((float)(maxwidth - width) / 2, (float)(maxheight - height) / 2);
if(canvas!=null){
...
//                System.out.println("speed:" + speed);
	...
	public void setwh(int w, int h, int s) {
		maxwidth=w;
		sh=s;
