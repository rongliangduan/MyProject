...
public class Result extends Activity {
	...
	public static String SRESULT_CODE = "9000";
	...
	/**
	 * @return String[] {resultStatusCode,resultStatus,memo,result,out_trade_no}
	 */
	public static String[] parseResult() {
		...
		try {
			String src = sResult.replace("{", "");
			src = src.replace("}", "");
			resultStatusCode = getContent(src, "resultStatus=", ";memo");
			if (sError.containsKey(resultStatusCode)) {
				resultStatus = sError.get(resultStatusCode);
			} else {
				resultStatus = "其他错误";
			}
			...
			out_trade_no = getContent(result, "out_trade_no=\"", "\"&subject=");
			isSignOk = checkSign(result);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return new String[] { resultStatusCode, resultStatus, memo, result,
				out_trade_no };
	}
